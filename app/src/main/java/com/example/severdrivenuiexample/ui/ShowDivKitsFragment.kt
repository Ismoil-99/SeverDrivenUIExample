...
import com.example.severdrivenuiexample.MainActivity
...
class ShowDivKitsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val divView = Div2View(Div2Context(baseContext = requireContext() as ContextThemeWrapper, configuration = createDivConfiguration()))
    ...
    private fun createDivConfiguration(): DivConfiguration {
        return DivConfiguration.Builder(PicassoDivImageLoader(requireContext() as ContextThemeWrapper,))
            .actionHandler(DemoDivActionHandler(findNavController()))
            .supportHyphenation(true)
            .visualErrorsEnabled(true)
            .build()
    }
    class DemoDivActionHandler(val nav:NavController) : DivActionHandler() {
